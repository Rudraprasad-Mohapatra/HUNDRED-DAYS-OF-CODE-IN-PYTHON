...
init = time.time() # current time before function runs
usingFor()
t1=time.time() - init
print("Time taken by for loop is ",time.time() - init)
init = time.time()
usingwhile()        
print("Time taken by while loop is ",time.time() - init)
print("Time taken by for loop is ", t1)
...
t = time.localtime()
formatted_time = time.strftime("%Y-%m-%d %H:%M:%S", t) #prints the time where the server code runs now it is in local disk 
